...
void CSimpleBuffer::Extend(uint32_t len) {
    ...
    //m_alloc_size = 4000;
    uchar_t* new_buf =  (uchar_t*)mp_calloc(m_mmpool, m_alloc_size);
    if(m_buffer) {
        //m_buffer[m_write_offset] = '\0';
        memcpy(new_buf, m_buffer, m_alloc_size);
    }
...
uint32_t CSimpleBuffer::Read(void* buf, uint32_t len) {
    ...
    //把 m_buffer + len处的内容移到 从 m_buffer 开始
    return len;
}
...
void CByteStream::WriteString(const char* str) {
    ...
    *this << size; //写到m_pSimpBuf 或者 m_pBuf中
