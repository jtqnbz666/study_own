...
#define HTTP_QUERY_HEADER "HTTP/1.1 200 OK\r\n"  \
                    "Cache-Control:no-cache\r\n" \
                    "Connection:close\r\n" \
                    "Content-Length:%d\r\n" \
...
char* GetErrorMsg(uint32_t error_code) {
    uint32_t index = error_code - ERROR_CODE_OFFSET; // 说明error_code是4000开始的
...
char* PackSendResult(uint32_t error_code, const char* error_msg) {
    ...
    snprintf(g_response_buf, MAX_BUF_SIZE, HTTP_QUERY_HEADER, content_len, json_str.c_str());
