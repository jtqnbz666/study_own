...
176.微信订阅：只要点进转盘就会发送订阅请求，第二天晚上八点会推送一次消息。 
...
``` SELECT channel, count(distinct "#account_id")
查某个渠道有多少玩家
from ta.v_user_11
...
4.快捷支付不会传giftid
3.东山再起就是入场付费礼包
2.破产结束触发东山再起，在VSJHandleGiftEvent中，豆子补给有3次(AVOID_LOSE_GIFT_POPTIMES)
...
5.单人匹配和自由组队都是发的teamstart(MSGID_TEAM_START | ID_REQ)。
