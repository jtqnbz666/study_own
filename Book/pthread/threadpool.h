...
{
public:
	//参数threaad_number是线程池中的线程是数量，max_requests是请求队列最多允许的等待处理的请求的数量
...
threadpool<T>::threadpool(int thread_number, int max_requests):
	m_thread_number(thread_number), m_max_requests(max_requests), m_stop(false), m_threads(NULL) {
		...
		//创建thread_number个线程，并将它们都设置为脱离线程p274
...
void threadpool<T>::run() {
	while(!m_stop) {
		m_queuestat.wait(); // 消耗信号量
		m_queuelocker.lock(); // 这步跟上一步不能交换位置， 否则死锁
