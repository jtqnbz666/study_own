...
1. 搜索云数据库RDS-->在实例列表中找到线上mysql实例并点进去-->左侧备份恢复-->根据需要选择一个时间节点并且点击右边的恢复--> 选择serverless一直点确定然后付款之后等待5分钟左右就完成了新实例的创建(210GB数据量)-->章节去实例列表拿到这个实例的地址，把staging的svc地址换成这个新地址(kubectl edit svc staging-mysql -n staging,修改externalName)-->删除原有redis缓存(flushdb)
~~~
...
~~~shell
# 背景：zset大小50000玩家，执行耗时很快(立马就执行完了)
...
/data/tools/build-server/src是给线上用的