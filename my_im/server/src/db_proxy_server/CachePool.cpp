...
bool CacheConn::hgetAll(string key, map<string, string>&ret_value) {
    ...
    if((reply->type == REDIS_REPLY_ARRAY) && (reply->elements % 2 == 0)) { //因为是key，value类型所以一定是偶数
        for(size_t i = 0; i < reply->elements; i += 2) {
            //keys是用户ID， values是更新时间
    ...
    while(m_free_list.empty()) {
        if(m_cur_conn_cnt >= m_max_conn_cnt) {
            ...
            if(ret) {
                ...
                m_cur_conn_cnt ++;
                log("new cache connection:%s, conn_cnt:%d", m_pool_name, m_cur_conn_cnt);
            }
        }
    }
...
int CacheManager::Init() {
    ...
    if(!cache_instances) {
        log("not configure CacheInstance");
        return 1;
    }
    char host[64];
	char port[64];
