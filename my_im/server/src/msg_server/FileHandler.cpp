...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu) {
    ...
    log("HandleClientFileRequest,%u->%u,fileName:%s, trans_mode:%u.", from_id, to_id, file_name, trans_mode);
    ...
    if(pFileConn) {
        ...
        pdu.SetSeqNum(pPdu->GetSeqNum());   //是一次数据，序号自然一样
        if(IM::BaseDefine::FILE_TYPE_OFFLINE == trans_mode) {
            pFileConn->SendPdu(&pdu); //离线传输模式，直接发
        } else {    //FILE_TYPE_ONLINE
            CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(to_id);
            if(pUser && pUser->GetPCLoginStatus()) { //已经有对端(to_id)账号的pc登录状态
                pFileConn->SendPdu(&pdu);
            } else {    //当前msgserv没有tto_id的pc登录状态， 需要向route_server查询状态
