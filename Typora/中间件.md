...
**etcd 存储以数据的 Revision 作为 key**，键值、创建时的版本号、最后修改的版本号等作为 value 保存到数据库。etcd 对于每一个键值对都维护了一个全局的 Revision 版本号，键值对的每一次变化都会被记录。
...
mysql中通过undolog来实现回滚和MVCC，事务一旦提交， undolog就没啥用了(**undolog主要是查询最新一条纪录，而etcd是根据以往的revision获取某条记录**)， 而etcd中涉及到对以前版本的查找(多，而且需要通过key(rivision)去找到指定的版本)，所以使用B+树提高查找效率
...
跟mysql的区别，mysql是行存储，hbase是列存储，mysql底层的文件系统就是我们一般的普通文件系统(ext2,ext3)来进行数据存储， 而hbase的底层文件系统用到是HDFS，也就是分布式文件系统
...
1.全局扫描。 2. 根据一个rowkey进行查询， 3.根据rowkey过滤的范围查询
...
![](E:\study_own\pic\20200725005228122.png)
