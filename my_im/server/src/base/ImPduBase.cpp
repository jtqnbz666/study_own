...
CImPdu::CImPdu() {
    ...
    m_pdu_header.flag = 0;
    m_pdu_header.service_id = 0;//SID_OTHER;
...
CImPdu* CImPdu::ReadPdu(uchar_t* buf, uint32_t len) {
    ...
    pPdu = new CImPdu();
    //因为CImPdu包含两个成员
    //1.CSimpleBuffer   m_buf;   m_pdu_header + PB序列化后的数据
    //PduHeader_t     m_pdu_header; // 头单独记录一下方便使用
    pPdu->Write(buf, pdu_len);  //这一句其实已经写好了m_buf了， 包含头和序列化后的数据
    pPdu->ReadPduHeader(buf, IM_PDU_HEADER_LEN);//这里只是把头单独记录一下
...
void CImPdu::SetPBMsg(const google::protobuf::MessageLite* msg) {
    ...
    m_buf.Write(szData, msg_size);  //把收到的数据放到m_buf中
    delete []szData;
    WriteHeader();  //写包头
}
