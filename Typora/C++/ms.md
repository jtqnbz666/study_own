...
2.如果自定义了拷贝/移动构造函数或拷贝/移动赋值函数、析构函数任意一个，编译器不会再生成移动构造和移动赋值函数。
3.默认构造和拷贝构造和拷贝赋值关系：如果定义了一个有参数的构造函数(拷贝构造函数也算)，那么编译器不会自动生成默认构造了，但拷贝/移动构造和拷贝/移动赋值还会生成，如果自定义了拷贝构造函数，虽然不会有默认构造、移动拷贝和移动赋值，但还是会自动生成拷贝赋值函数。自定义拷贝赋值函数同理。
