...
struct socket {
	uintptr_t opaque;			//actor地址 对应的结构
	struct wb_list high;
	struct wb_list low;
	int64_t wb_size;
...
struct socket_server {
	...
	int checkctrl;
	poll_fd event_fd;				//epoll每个函数的第一个参数efd，就是通过epoll_create创建的那个
	ATOM_INT alloc_id;
	int event_n;
	int event_index;
	struct socket_object_interface soi;
	struct event ev[MAX_EVENT];					//收到的事件的数量最大值
	struct socket slot[MAX_SOCKET];				//这个结构体就是actor与fd建立关系的结构体
	char buffer[MAX_INFO];
	uint8_t udpbuffer[MAX_UDP_PACKAGE];
	fd_set rfds;				//在select中用于检测读事件的东西， 在这是为了判断管道recvctrl_fd是否有消息
};
...
struct request_resumepause {				//有点slot的意思了
	int id;
	uintptr_t opaque;
};
...
struct socket_server * 
socket_server_create(uint64_t time) {			//初始化
	int i;
	int fd[2];
...
new_fd(struct socket_server *ss, int id, int fd, int protocol, uintptr_t opaque, bool reading) {
	...
	if (sp_add(ss->event_fd, fd, s)) {		//去socket_epool.h里找，不是socket_kqueue.h
		ATOM_STORE(&s->type, SOCKET_TYPE_INVALID);
		return NULL;
	}
...
open_socket(struct socket_server *ss, struct request_open * request, struct socket_message *result) {
	...
	for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next ) {
		...
		sp_nonblocking(sock);
		status = connect( sock, ai_ptr->ai_addr, ai_ptr->ai_addrlen);//连接外部服务
...
resume_socket(struct socket_server *ss, struct request_resumepause *request, struct socket_message *result) {
	...
	struct socket_lock l;		//为什么要加锁， 为了提供多线程写数据
	socket_lock_init(s, &l);
	if (enable_read(ss, s, true)) {   //epoll_ctl ， 对这个socket的事件进行修改
	...
	if (type == SOCKET_TYPE_PACCEPT || type == SOCKET_TYPE_PLISTEN) {
		...
		s->opaque = request->opaque;								//找到了， 就是这里改变的sockfd 与 具体服务的绑定
...
// return type
static int
ctrl_cmd(struct socket_server *ss, struct socket_message *result) {			//这个result什么也没传进来，是为了传出去的
	int fd = ss->recvctrl_fd;		//管道的读端
	...
	int type = header[0];
	int len = header[1];
	block_readpipe(fd, buffer, len);			//这里的buffer可以认为里面的id就是fd，opaque就是handle， 可以把buffer认为是对方的服务节点
	// ctrl command only exist in local fd, so don't worry about endian.
	switch (type) {
				//把buffer收到的东西  部分赋值给result
	case 'R':
		return resume_socket(ss,(struct request_resumepause *)buffer, result);		//return SOCK_OPEN
	case 'S':
	...
	case 'O':
		return open_socket(ss, (struct request_open *)buffer, result);				//return SOCK_OPEN,连接第三方服务
	case 'X':
		result->opaque = 0;
		result->id = 0;
		result->ud = 0;
...
report_accept(struct socket_server *ss, struct socket *s, struct socket_message *result) {
	...
	int client_fd = accept(s->fd, &u.s, &len);			//s->fd就是listenfd
	if (client_fd < 0) {
	...
	int id = reserve_id(ss);	//一个经过一定算法得到的id
	if (id < 0) {
		close(client_fd);
		return 0;
	}
...
socket_server_poll(struct socket_server *ss, struct socket_message * result, int * more) {
	for (;;) {
		if (ss->checkctrl) {
			if (has_cmd(ss)) {		//这里面是select多路复用, 意思就是用select来检测管道
				int type = ctrl_cmd(ss, result);  // ctrl_cmd		管道
				if (type != -1) {
		...
		if (ss->event_index == ss->event_n) {
			///客服端的所有网络信息全部来自这里
			ss->event_n = sp_wait(ss->event_fd, ss->ev, MAX_EVENT);  	//sp_wait		
			ss->checkctrl = 1;
			if (more) {
				*more = 0;
			}
		...
		struct socket *s = e->s;			//意为 ： 每个事件都有对应的slot数组结构，这个s对应的是ev.data.ptr
		if (s == NULL) {		//仔细看1655行，s的值就是这里来的
			// dispatch pipe message at beginning
			continue;
		}
		struct socket_lock l;		//因为写事件多个actor都会触发，所以必须加锁，保证多线程下的写事件
		socket_lock_init(s, &l);
		switch (ATOM_LOAD(&s->type)) {
		case SOCKET_TYPE_CONNECTING:	//服务器连接第三方，第一次可读事件
			return report_connect(ss, s, &l, result);
		case SOCKET_TYPE_LISTEN: {					//listenfd上的可读事件
			int ok = report_accept(ss, s, result);		//此时这个result是空的，传进去是为了获得东西
			if (ok > 0) {
...
send_request(struct socket_server *ss, struct request_package *request, char type, int len) {
	...
	for (;;) {
		ssize_t n = write(ss->sendctrl_fd, req, len+2);			//往管道写数据
		if (n<0) {
			if (errno != EINTR) {
...
socket_server_start(struct socket_server *ss, uintptr_t opaque, int id) {
	struct request_package request;
	request.u.resumepause.id = id;		//id是listenfd									有没有发现这两个就是slot数组的内容也就是struct socket
	request.u.resumepause.opaque = opaque;			//这个参数是handle也就是source
