...
21.查看有多少条数据不用select count(*)，可以show table status like '%tablename%'，还可以看到这张表占用的大小（Data_length+Index_length）等信息，但这样不一定准。比如克隆库的时候行号不一定对。
...
``` sql
ALTER TABLE card_god_info_all
ADD COLUMN common_progress_v2 varchar(1024),
ADD COLUMN elite_progress_v2 varchar(1024),
ALGORITHM=INPLACE, LOCK=NONE;
# 原理：
执行alter时，有三份数据，一份是旧的idb文件，一份是新的idb文件(执行alter瞬间加锁记录旧idb快照，不断读取旧idb内容放到其中)，一份是增量日志， 此时有一条update操作过来时会更新旧的idb文件，新的idb文件不会更新，并且会把操作记录到增量日志中，当新idb同步完旧idb数据时候，加锁(一般比较快)，不再处理新的sql操作，然后新的idb文件会同步alter执行期间发生的操作(增量日志中有记录），然后用新的idb文件替换旧的idb文件，接着释放锁，处理后续请求。
# 与ALGORITHM=copy的区别
inplace是在innodb存储引擎层对idb文件进行操作，而copy是在sever层，它不关心idb的存在，可以理解为它在server逻辑层执行了 INSERT INTO `newtable` SELECT *, 'new filed' FROM `oldtable`;
# 说明：
不关是copy还是innodb，增加字段过程都会临时造成磁盘占用翻倍的情况。
```
