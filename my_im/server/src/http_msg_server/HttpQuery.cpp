...
void http_query_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam) {
    ...
    if(year != g_last_year || mon != g_last_month || mday != g_last_mday) {
        //新的一天来了， 清空计数
        log("new day comming, g_total_query=%u", g_total_query);
        g_total_query = 0;
        g_last_year = year;
        g_last_month = mon;
        g_last_mday = mday;
...
void CHttpQuery::_QueryCreateGroup(const string& strAppKey, Json::Value& post_json_obj, CHttpConn* pHttpConn) {
    ...
    try {
        ...
        msg.set_group_avatar(group_avatar); //化身
...
HTTP_ERROR_CODE CHttpQuery::_CheckPermission(const string& strAppKey, uint8_t nType, const list<uint32_t>& IsToId, string strMsg) {
    strMsg.clear();
    return HTTP_ERROR_SUCCESS;
}