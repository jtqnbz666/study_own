...
class processpool {
private:
	//将构造函数定义为私有的，因此我们只能通过后边的create静态函数来创建processpool实例
	//单例模式
...
//用以处理信号的管道， 以实现统一事件源，后面称之为信号管道。
static int sig_pipefd[2]; //全局唯一，信号统一处理
...
static void sig_handler(int sig) {
	int save_errno = errno; //为了保证可重入
	int msg = sig;
	send(sig_pipefd[1], (char*)&msg, sizeof(msg), 0);		//只负责通知父进程
	errno = save_errno;
}
...
static void addsig(int sig, void(*handler)(int), bool restart = true) {
	...
	if(restart) {
		sa.sa_flags |= SA_RESTART; //如果系统调用被这个信号打断了，会再次进行系统调用
	}
...
processpool<T>::processpool(int listenfd, int process_number) : m_listenfd(listenfd), m_process_number(process_number), m_idx(-1), m_stop(false) {
	...
	for(int i = 0; i < process_number; i ++) {
		int ret = socketpair(PF_UNIX, SOCK_STREAM, 0, m_sub_process[i].m_pipefd); //可双向
		assert(ret == 0);
		m_sub_process[i].m_pid = fork();   //生成子进程的时候把m_pid与子进程绑定
		assert(m_sub_process[i].m_pid >= 0);
		if(m_sub_process[i].m_pid > 0) {
			close(m_sub_process[i].m_pipefd[1]);   //一般默认用管道的1进行写，但这里是双向的
			continue;
		} else {
...
void processpool<T>::setup_sig_pipe() {
	...
	addfd(m_epollfd, sig_pipefd[0]); 	//子进程通过1写， 用0 来监听
	//sig_pipefd是静态的， 全局就一份

	//设置信号处理函数
	addsig(SIGCHLD, sig_handler);
	addsig(SIGTERM, sig_handler);
	addsig(SIGINT, sig_handler);
	addsig(SIGPIPE, SIG_IGN); //对端读关闭的情况
}
//父进程m_idx值为-1， 子进程中m_idx值大于等于0， 我们据此判断接下来要运行的是父进程代码还是子进程代码
template<typename T>
void processpool<T>::run() {
	if(m_idx != -1) {	//m_idx 表示的是父进程
		run_child();
		return ;
	}
	run_parent();
...
void processpool<T>::run_child() {
	setup_sig_pipe();  //因为进程间是不会继承信号处理函数的，所以每个新的进程都要重新设置
	...
	while(!m_stop) {
		...
		for(int i = 0; i < number; i ++) {
			...
			if(sockfd == m_listenfd) {
				//如果有新的连接， 就用Round Robin方法将其分配给一个子进程处理
				...
				if(ret <= 0) {
					...
					for(int i = 0;i < ret; i ++) {
						switch(signals[i]) {
							case SIGCHLD: {
								...
								while((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
									for(int i = 0; i < m_process_number; i ++) {
										//如果池子中第i个子进程退出了，则主进程关闭相应的通信管道，并设置相应的m_pid为-1， 标志该子进程退出。
							...
							case SIGTERM:
							case SIGINT: {
								//如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束，当然， 通知子进程结束更好的方法是向父子通信之间管道发送特殊数据
