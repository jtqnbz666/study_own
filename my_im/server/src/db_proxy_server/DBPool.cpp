...
CDBManager* CDBManager::s_db_manager = NULL;    //单例模式 懒汉
...
int CResultSet::GetInt(const char* key) {
    ...
    if(idx == -1) return 0;
    else return atoi(m_row[idx]);
}

char* CResultSet::GetString(const char* key) {
    int idx = _GetIndex(key);
    if(idx == -1) return NULL;
    else return m_row[idx];
}
...
bool CPrepareStatement::Init(MYSQL* mysql, string& sql) {
    ...
    m_param_cnt = mysql_stmt_param_count(m_stmt); //返回? 的个数
    if(m_param_cnt > 0) {
...
void CPrepareStatement::SetParam(uint32_t index, string& value) {
    ...
    m_param_bind[index].buffer_length = value.size(); //string类型就需要这句
}
...
uint32_t CDBConn::GetInsertId() {
    return (uint32_t)mysql_insert_id(m_mysql);//获取最近的自增id
}
...
//增加保护记住， 把分配的连接加入另一个队列， 这样获取连接时， 如果没有空闲连接。
...
void CDBPool::RelDBConn(CDBConn* pConn) {
    ...
    m_free_notify.Signal();//通知现在有一个空闲连接了
    m_free_notify.Unlock();
}
...
int CDBManager::Init() {
    ...
    FILE* fp = fopen("dbproxyserver.conf", "r");
    if(!fp) {
        CConfigFileReader config_file1("routeserver.conf");
        config_file = config_file1;
    } else {
        fclose(fp);
        CConfigFileReader config_file2("dbproxyserver.conf");
...
void CDBManager::RelDBConn(CDBConn* pConn) { //manager有一个reldbconn
    if(!pConn) return ;
