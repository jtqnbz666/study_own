...
//		printf("Server Version: MsgServer/%s\n", VERSION);
	...
	char* str_max_conn_cnt = config_file.GetConfigName("MaxConnCnt");	//100000
   ...
   // serv_info_t* push_server_list = read_server_config(&config_file, "PushServerIP",
	...
	uint32_t db_server_count2 = db_server_count * DEFAULT_CONCURRENT_DB_CONN_CNT; //4
	char* concurrent_db_conn = config_file.GetConfigName("ConcurrentDBConnCnt");	//配置文件是2
	if (concurrent_db_conn) {
		concurrent_db_conn_cnt  = atoi(concurrent_db_conn);  //2
		db_server_count2 = db_server_count * concurrent_db_conn_cnt; //4
	}
	...
	for (uint32_t i = 0; i < listen_ip_list.GetItemCnt(); i++) {
		ret = netlib_listen(listen_ip_list.GetItem(i), listen_port, msg_serv_callback, NULL);	//这里就已经把listenfd放到epoll_wait中了
		if (ret == NETLIB_ERROR)
			return ret;
	}
