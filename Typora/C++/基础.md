...
~~~c++
1.以字节为单位，计算数组元素个数可以用 sizeof(array) / sizeof(array[0])
2.对于字符串变量，sizeof会计算终止符\0，比如sizeof("Hello\n")为7，但如果是string类型的size方法不会计算\0，结果为0
~~~
...
# 小细节
2.未初始化的变量会放到.bss中可以有效减少elf文件的大小，比如int arr[1000];虽然size命令可以看到.bss变大了，但ll命令可以看到文件大小没有变化，如果写成arr[1000] = {1};就会放到.data段(如果是 = {0};现在的编译期同样认为是未初始化会放到bss段)，并且会增加elf文件的大小，因为bss段实际只会记录该变量的大小，不会存储具体的值，但在装载期放进内存时候还是会占用1000个int空间，并且把值全部初始化为0。
...
2. g++ -S a.cpp 
可以看到汇编语言，可以看到某个变量在.bss还是.data还是.rodata
~~~
