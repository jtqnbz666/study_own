...
CSyncCenter::~CSyncCenter() {
    if(m_pLockGroupChat != NULL) {
        delete m_pLockGroupChat;    //锁
    }
    if(m_pCondGroupChat != NULL) {
        delete m_pCondGroupChat;    //信号量
    }
}

void CSyncCenter::getDept(uint32_t nDeptId, DBDeptInfo_t** pDept) {
    //为什么传二级指针，想一想二叉树传入空指针引发的问题，这里也可以改成传引用
...
void CSyncCenter::startSync() {
    //开启了一个
    pthread_create(&m_nGroupChatThreadId, NULL, doSyncGroupChat, NULL);
}
...
//就是获取total_user_updated和last_update_group字段
void CSyncCenter::init() {
    CacheManager* pCacheManager = CacheManager::getInstance();
    CacheConn* pCacheConn = pCacheManager->GetCacheConn("unread");
    if(pCacheConn) {
        string strTotalUpdate = pCacheConn->get("total_user_updated"); //string类型
        string strLastUpdateGroup = pCacheConn->get("last_update_group"); //string类型
...
//同步群组信息
//就是将IMGroup表中群组信息的变化(比如增加删除)，将信息同步到IMRecentSession表中
//从而就可以更新会话列表的信息了
void* CSyncCenter::doSyncGroupChat(void* arg) {
    ...
    do {
        ...
        CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");//从数据库
        if(pDBConn) {
            //从IMGroup表获取数据
            //以m_nLastUpdateGroup时间点做比对
            ...
            if(pResult) {
                while(pResult->Next()) {
                    ...
                    if(nLastChat != 0) {
                        mapChangedGroup[nGroupId] = nLastChat;//这里只是记录
                    }
                }
                delete pResult; 
            }
        ...
        for(auto it = mapChangedGroup.begin(); it != mapChangedGroup.end(); it ++) {
            uint32_t nGroupId = it->first; //id
            list<uint32_t> lsUsers; //存储群组所有用户id
            uint32_t nUpdate = it->second; //最后更新时间
            ...
            for(auto it1 = lsUsers.begin(); it1 != lsUsers.end(); it1 ++) {
                ...
                //这个nSessionId字段就是IMRecentSession表中的id字段
