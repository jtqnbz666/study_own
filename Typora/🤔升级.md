...
43.济南电赛三副牌的房间配置上5300，直接改缓存, 四副牌5900,经典5000
...
game服务：SelectGamesvrdV2(在automatch)，选最小压力的
...
3.比如两个玩家都是直接点快速开始游戏(类型1)，会先放到mapAutoMatchUsers，然后定时器把实力相近的取出来组成临时队伍(两人一组)放到mapAutoMatchTempTeamUsers，私人队伍(创建了房间自己开始，类型2)或者双人组队(类型3)会放到mapAutoMatchTeamUsers中
...
2.mapTeamMatch存放了所有队伍信息，有单人也有双人，每秒会执行DealTeamTimerFunc检查是否有队伍准备完毕，或者需要移除队伍(比如长时间未准备)，如果都准备了就发送MSGID_TEAM_MATCH_TABLE | ID_NTF给room服务。

automatch：
根据roomid选择一个人数最少的game地址，在DealAutoMatchTable设置了SJ_MATCH_390892，包含game地址, 然后会发送（MSGID_MATCH_TABLE | ID_NTF）通知前端匹配成功(实际匹配操作在room做的)
~~~
...
6.匹配成功后才有tableid
5.prematch服务上：KEY_FKLD_T_tableid（记录桌子信息），KEY_FKLD_MATCH_390892（记录玩家tableid，room服务地址，这里面的game服务地址是在room服务算出来的），KEY_FKLD_ROOMSVRD_TM、KEY_FKLD_ROOMSVRD_UM分别记录prematch服务上的tableid和userid，
4.team服务上：KEY_FKLD_TEAM_teamid（记录队伍信息），KEY_FKLD_TEAM_MATCH_3890892（记录玩家的teamid，team服务地址），KEY_FKLD_TEAMSVRD_TM、KEY_FKLD_TEAMSVRD_UM是set对象，分别记录team服务上的teamid和userid，
3.prematch是随机选择的
