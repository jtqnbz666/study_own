...
~~~
小游戏相关：微信、淘宝、oppo、抖音都接入了SDK登录，其中微信、oppo小游戏可以切仿真，抖音和淘宝无法切仿真，有域名白名单限制(只能配域名)
~~~

### 渠道相关

``` 
华为：渠道huawei，组huawei， 鸿蒙：渠道huaweihm，组huawei，荣耀：渠道honor，组honor

小游戏：
taobaomini', 'wechat', 'oppomini', 'douyinmini'
```

服务通讯

``` shell
1.OnMsg用于收到客户端端的消息 (只有连接服有)
2.RecvProc用于服务器之间收到消息后的回调
3.OnReceiveQueueMsg从消息队列里面拿消息后的处理函数
```
...
221.微信小游戏切仿真需要先进入调试模式(右上角三个点)， 但切内网则不用，因为内网的ip在后台配置了。
220.前端http如果超时会新起一个连接。
219.qxgz座位号按照多乐号从小到大
218.小游戏登录走的负载均衡。
217.所有服务都是在OnReceiveQueueMsg接收数据
216.流通值是输赢的绝对值 + 房费的绝对值。
215.非会员的会员大礼包一天只弹一次。
214.苹果华为都是自动更新，所以新包上线后转化的很快，抖音这种没有应用商店，要么在直播间小手柄下载，要么登录的时候提示他后台拉取资源下载。
213.无附件邮件读取或者领取奖励后，时间设置为第二天23:59:59
212.会员多送是在多送后再多送，比如50元买52w豆，在52w的基础上多送。
211.datasystem.db_duole_user_all，玩家的个人
210.机器人和真人的special一定一样。
209.客户端数数上报直接上报的线上
208.三打哈是找队友的逻辑，但是上报的是找对手。
207.大厅快速开始，有最近就传最近的玩法，没最近就用登录的recommendplaytype
206.头像框展示时间是用本地时间计算的(背包不是)， 比如，服务器会发送一个获取时间gettime和一个保持时间keeptime，前端把二者相加再减去本地时间计算最终时间。
205.SJ_MATCH_游戏状态，2为对局中，4为逃跑。
204.象棋里玩升级，计费点不会算到升级中，不能有付费任务。
203.大转盘，鸿蒙只让每天拉取一次，wechat_mini_turntable_draw_v2
202.录像可以指定视角观看，复制的牌局多乐号谁在前，主视角就是谁，默认0号
201.首充协议firstcharge/check每次登录都会拉，里面记录了玩家是否需要领取首充奖励，然后第一次付费后会再拉取一次，客户端知道已经领取了奖励，下次付费就不会拉取了，也没有存在本地，因为每次登录都会拉一次。
200.小游戏进入成功的标志是/smallgame/verify/v1
199.渠道号是定死的，跟登录方式没关系，比如官包登录都是duole，然后appstore的微信登录渠道为appstore。
198.微信小游戏和微信登录是相同的多乐号。
197.验证签名的digest必须需要在body立马放值才会有。
196.线上问题直接grep查
195.匹配找机器人进去就开始计算时间，20s就能找到，还有一个条件是准备，比如有30s，已经等了25s，没准备就没机器人。准备了机器人会立马来。
194.表情的4001 - 4016是会员表情，3001 - 3020是普通表情
193.首充检测每次登录都会检查firstcharge_check
192.微信小游戏的设备id是随机的，卸包会变。
191.sjgame的isallteam检测用的数据是redis缓存的值，比如临时队伍，但是判断某个玩家的teamtype时他其实是单人队伍类型。sjroom的临时队伍类型发送SendAutoMatchMSG时用的是玩家的原始队伍类型，然后sjgame会把原始队伍类型设置到SJ_T中，然后isallteam判断时用的teaminfo数据是从SJ_T拿到的，所以临时队伍的队伍类型依然是单人队伍。
180.附属游戏(比如捕鱼)下单：附属会把参数传给升级客户端，然后升级请求自己的服务器走下单逻辑。
179.房间服重启搜索ClearRoomSvrdSet看影响了哪些玩家
178.搜索“升级队伍服务日志”看玩家最近日志
177.座位号顺序规则：相同匹配等级下，首先是成对规则，队伍的主多乐号为多乐号小的人，然后队伍和队伍之间，队伍的主多乐号小的在前，也就是说54963和54979成队，54963为主号，54964和390892成队，54964为主号，然后呢54963队伍比54964队伍小，所以54963为0号座位，54964为1号座位。前提是匹配等级一样，一个匹配队列中用的map，多乐号从小到大找，如果匹配等级不一样，那么主号就不一定了，因为是浮动找的，可能是多乐号大的人去找下一等级队列多乐号小的人，主号就是大的人的多乐号。匹配人太多也不一定，因为出了第一队，后边队的顺序被随机排序了(DealFindTeamTimer)。
176.微信订阅：只要点进转盘就会发送订阅请求，第二天晚上八点会推送一次消息，但不是一定会推送的，具体要问微信的机制，然后呢有时候游戏中的订阅标识会消失需要重新订阅，可能微信那边有什么机制过段时间就没有订阅了。 整体逻辑是，订阅一次，微信推送一次，推送之后，需要重新订阅。
175.如果是符合陪玩的人，自由组队不会去加入别人的队伍，而是创建一个队伍，然后等机器人(扶持或收割)进来。
174.自由组队如果是机器人陪玩，真人会被排序放在第一位，如果都是真人，多乐号小的在前边。
173.赢太多，无法收割，删除SJ_HELP_AI_USER_WIN_MONEY_
172.非连打不散桌，打过A后会重新亮主定庄。
171.临时队伍和单人匹配是一类队伍、自由组队和牌友邀请是另一类的，两类不能混在一起匹配
170.定庄无法改级牌，只能改第一局。
169.账号解封在retool的够级平台相关中，修改账号状态，状态1为封号，3是注销，0正常
168.主播有个混淆开关，如果不开也不会混淆，是否混淆是前端决定的
167.旧版离开发team_chage，新版离开先发leave_game再发team_start
166.离开统一成了一个，MSGID_LEAVE_GAME | ID_REQ，leave_team没用了
165.私人场不操作解散时间SJ_PRIVATE_DISSOLVE_TIME
164.搜索DealAutoMatch userid找开始匹配时机，终止匹配搜索NTFTeamMatchInterrupt，可能是主动请求MSGID_LEAVE_GAME，也可能是offline断线等。
163.某个房间如果还有人，就在enteroom先拦截，
162.多乐币变化会发送/account/info/change通知更新
161.房间配置，PLAYTYPE_INFO_LIST，如果只改version则前端提示房间配置更新，如果加上描述，则展示描述,updatetime表示展示到啥时候
160.不散桌，房间配置不会更新，只有再第一次开局时调用GetTableInfo加载房间配置信息,
159.捕鱼收入从公共数据看板->单游戏数据看板(最下边的附属捕鱼数据)->筛选渠道->三个点导出数据
158.房间更新提示：PLAYTYPE_INFO_LIST，不支持单房间，对整个玩法生效
157. datasystem.ods_platform_login能看到设备型号
156.录像存储的上传下载之前用的是同一个域名，如果下载的域名被封禁了，会影响上传业务，现在改为上传和下载用单独的域名
155.登录过期时间为2天
154.审核中关闭是产品处有一个开关，审核中的时候前端拉取到的值是关闭的就不展示
153.捕鱼主播包前端自己限制的
152.机器人豆变化投放产出搜索debt_type_robot_game，从debt_表筛选这个类型可以得到输赢
151.清号去新手场的机器人不会亮主
150.初始积分是9，从gamelog的score_chg可以看到变化
149.optioncfg中的matchend控制一局散桌
148.天命加倍："doublelist": "[{\"doublemax\": 10, \"doublemin\": 10, \"weight\": 1}]",
147.举报需要有两人、三人举报才有效
146.app反馈走sdk，小游戏走我们
145.喇叭需要把总局数改大一点
144.看发牌情况搜索shoupaiStr
143.双扣机器人，启动VSKAUTOMATCH
142.userinfo可以看到最近一次登录时间
141.清除账号没有清牌龄
140.充值多送区分了多乐币和付费
139.首充后，首充奖励前端会自动请求
138.好评100牌龄限制，只有华为推送
137.留言反馈是请求的中台，时间以中台为准
136.扶持机器人不会亮主
135.内网负载均衡：nslookup sandbox-shengji.duole.com
134.前端APP和小游戏web请求超时都是5s

132.没散桌，级牌连打
131.room仲裁服的路由是SJ_MSG_QUEUE_ROOMSVRD，但它现在只处理对team服务的消息转发，不对room服务转发。
130.单人开始游戏发teamstart，如果是多人依靠的是都点了准备，服务器自动发MSGID_TEAM_MATCH_TABLE | ID_NTF给room开始匹配
129.同一个多乐号可能在不同的渠道登录，比如app store的微信登录和小游戏微信登录是同一个多乐号，但渠道不一样。
128.内网脚本日志sudo find / -name '*VSJHandleTaskMonitor*'
127.主播视角，开了混淆，看别人只有头像和昵称是修饰过的，别人视角看主播，昵称，头像，游戏豆（到达某个值），积分这些都是修饰过的。
126.查账单升级用的发货时间，如果订单的创建时间不是当月的，则查不到，需要改为创建时间
...
120.游戏结束后如果组成临时队伍，会给前端先发踢人消息kick_reason_auto_match离开这张桌子，再发MSGID_TEAM_MATCH_TABLE | ID_NTF告诉room重新开始匹配(team全部准备通知room队伍匹配也是发的这个)，进一步发MSGID_MATCHING | ID_NTF告诉前端已经开始匹配。
...
113.desc表中is_return含义：0送，1收，2小游戏，4.fkld， 增加描述信息，all_debt_desc和debt_desc都得加
...
103.豆结算时公共部分都是一样的(亮主、炒底、大光，这些支持加法或者乘法)，加倍每个人都是不同的并且只能是乘法，如果有个对家加倍，对家一共加倍是1.5倍(不加倍的是1倍，加倍是2倍)，因为会除2，
102.circulate流通指一局总的赢豆数量(也就是总输豆数量)
101.2024年的gamelog的grabescore不全，用play_room的score
100.下单时写purchase表、purchasepresent(礼包才会写)，如果订单支付成功收到平台的回调时会写purchase_record表和buy表，如果buy表有记录，而purchase_record没有则可能是补单逻辑
98.小于2000最低准入就是破产
...
71.GetSecurityLevel高敏感针对多乐号固定返回1，小游戏用的是iplimit
70.
...
42.线上存储桶是bi-1252509288/json-logs,内网是test-duole中的fluent-logs，录像存储桶是duole-res-playback03-1252509288/playback
...
19.mysql_shengji.play_room可以看到每局的分数变化,并且录像只有一条，不像
...
3.c服务上执行python shengjirobotgameinfo.py恢复机器人
...
2.头像框增加时长。
select "#account_id", event_time, prop_id,  prop_name, keep_time from ta.v_event_5 WHERE "$part_date" >= '2025-05-12' and "#event_name" = 'frameDebt' and "#account_id" = '64242'  and keep_time >= 45000000  order by "#event_time" 
```

七星高照重要逻辑

``` 
DealVolatileTurn: 通知操作token
DealVolatileTokenTurn: 操作后(比如胡)调用，全部操作完成后调用UpdateVolatile结算玩家操作，此时会设置为等待动画的状态
HandClientAnimationEndREQ: 前端动画结束后。

tips：
4.破产陪玩一天只有一局，优先触发生涯陪玩。
3.硬胡流水只有一条，但是页面上飘的豆信息旧版有两条，新版(2.19.1)合并成了一条
2.听牌提示有两部分(点牌，右边的灯泡)，右边的灯泡优先拿-1，如果没有-1就从右往左找第一个。
1.可炸可过，如果点击过，超时后直接出牌(胡之前)
2.第一轮不出牌基本上是主动托管(并且是首轮主动托管)，后边的流程如果是需要玩家自己操作的但是没有操作并且在托管状态则是被动托管

有限责任制（跟升级一样）：
1.自摸：赢的倍数对于三人来说肯定都一样，比如自身携带100w，理论赢300w，那么三人平均给我33.3w，如果有人只有32.3w，那么还有1w由另外两个人一人给我5k，因为她们本来理论输100w(33.3w+5k小于100w)。
2.点炮，输A玩家倍数为2，输B玩家倍数为1，那么比如输30w，自身也够30万，就给A20w，给B10w，按比例分，自身不够也按比例分，如果自身只有10w，但B玩家只有1w，所以B只能赢1w，还剩9w，9w小于A玩家能赢的20w，则这9w都给A玩家。
3.如果有人有免输，并且跟他一起输的人金币不够，比如A还欠10w，B和C帮忙一人补5w，C有免输，则B自己补5w。
```
...
``` 
5.最后三天买精英证时会提示即将过期。
4.前端写死最后七天每天展示一次"还剩n天结束"
3.初始等级为1，并且奖励在初始化时就设置为可领取状态了。
2.CalCardGodGrade.py处理变化，直接用grade访问，比如grade为1，访问时候修改的其实是第二个元素。
1.当前只有没买精英证并且达到60级才发邮件
```

临时队伍流程

``` 
四个人结束后如果其中有一个人退出(OnUserLeaveGame)，那么桌子会把退出的人以及他的队友踢出桌子，从m_GameData.allPlayerInfos.中移除，并且会通知这两个人恢复单人队伍(OnUserLeaveGame中的NotifyRecoverSingleTeaminfo)，接着会进入CheckFloatRoom，此时另外两个人如果是准备状态会触发CreateTempteam创建临时队伍，通知房间服重新匹配，房间服高速客户端进入匹配状态。
```

匹配功能

~~~shell
mRoomMentalityWeight 心智段位，某写房间是用匹配分计算的段位
~~~
...
# 连打中断
with needseq as (
    select stageseq from shengji.gamelog where date >='{{date.start}}' and date <= '{{date.end}}' 
    and room_id = {{玩法房间}}
    group by stageseq
    having count(*) = 20 -- 一人4条记录，5轮打满20条记录 
), 
# escapetype逃跑类型，1主动，2被动
# resulttype 1赢，0输（用moneychg也行
```

牌友功能

~~~ 
突破：会给连接服发送MSGID_BREAK_THROUGH_CARD | ID_REQ，进一步发给web的SJ_USE_BT_CARD_LIST
~~~
...
~~~shell
条件：清号，修改牌龄为0
配置：OME_GIFT_CFG，抖音渠道配置是WELCOME_GIFT_CFG_DOUYIN
~~~

### AI功能

~~~shell
5.陪玩AI不会主动去亮主、炒底，其他AI豆会
4.AI最终调用的类型只有三种（陪玩、弱收割、强收割，sjgame搜enum AIType），不管是哪种AI最终都会是这三类中的一种，比如凑桌和连打陪玩AI最后是用的强收割。
3.当前在用的AI类型只有陪玩(5)、弱收割(7)、收割(6)、凑桌(过A场)(1)、连打陪玩(8)。 #新手的陪玩AI(2、4)去掉了(搜SJ_NEW_USER_，__CardNum始终返回0)，
2.经典的AI区分了强弱，炒底当前只有一种AI（收割AI），炒底陪的时候用的是发牌策略，把好牌给玩家。
1.AI匹配不上时会放到正常的队伍中匹配，就不会尝试去匹配AI了。

单人：
陪玩AI：（配置为NEW_USER_CFG）
DealRobotAutoMatchTimer1---(1201，两个单人同时匹配
DealRobotAutoMatchTimer2--- (陪玩AI，1201，重置号去新手, 牌龄改为0，）
DealAutoMatch3V1Timer1---(三打哈）
扶持AI：(配置为HELP_AI_CFG)
DealHelpAIRobotAutoMatchTimer1---（扶持AI，1101， 
收割AI：(配置为HELP_AI_CFG)
DealAdvancedAIRobotAutoMatchTimer1---（收割AI，1102，开个会员
智能AI：（配置为AI_USER_CFG）
DealAIRobotAutoMatchTimer1---(1202，两个单人同时匹配
DealAIRobotAutoMatchTimer2---(智能AI，1202,牌龄改为50，
队伍：
智能AI：
DealTeamAutoMatchTeamAIRobotTimer1--- (1202，多乐号小的人必须是符合智能AI的
扶持AI（未开）
DealTeamAutoMatchHelpAITimer1---（写死
收割AI（未开）
DealTeamAutoMatchAdvancedAITimer1---(写死
~~~
...
~~~shell
2.任务跳转
1.任务进度是通过修改SJ_DAYINFO完成的。
2.同类任务当前只会展示一个，添加测试任务需要去掉这个限制if task_type in list_type
~~~

### 回归功能

~~~shell
4.牌龄大于7则回归，如果小于7，该多乐号是某个设备第一次注册的账号则不是回归，如果不是某个设备第一次注册的账号则是回归，是否是某设备首次注册查询device_limit表，有一个user_id
3.回归七日礼，如果忘记领了则不能再领了
2.修改SJ_LOGIN_390892的logintime为21天前即可即可，如果没有此缓存，就改SJ_GI_390892的lastlogintime，也没有就改sql的userinfo表的lastlogintime。
1.超过21天未登录就是回归用户，回归礼有效期有14天
~~~
...
~~~shell
13.非会员的等级是0
12.续费会员如果当时已经是会员身份则不会加成长值，如果会员已经过期会根据当前买的会员类型来加(月10年15)
11.成长值如果因为改服务器时间增加了，时间调回来后不会减回来，因为有个最近的更新时间。
10.试用会员当前不支持重复获得。
9.会员最多买2年，按照31/月计算，shop_buy中可以看到。
8.如果超过最大购买期限，每一个月会被转为20成长值，余数一天为1成长值
7.会员结束日期写的是2号，则3号才过期，4号开始扣成长值，如果直接改服务器时间为5号，会扣2天(4、5号)的成长值
6.之前的周会员看起来不会加成长值
5.过期会员/过期试用会员，成长值每天掉30点.
4.31天为1个月，比如1号买两个月，会加61天，因为会算上当天，比如是62号，63号才过期，年会员也一样会减一天，因为算了当天，比如curday + 31*12-1天
3.不管是什么会员，过期了都会设置type为0，不存在viplevel为0的会员信息，过期会员过期后至少也是1，（试用会员除外，类型为5，并且viplevel会变为0
2.ReqsShopBuy购买会员会有邮件通知
1.会员是31天为一个周期，首次(不区分月、年)买会员送10点(年送15点)， 每天加10成长值(年加15)，之后每次买都是续费，如果买的是年会员直接送200点(必须买>=12月，不能是累计,比如一次性买24个月就加400，如果先买11再买1个月虽然是年会员但不加成长值)，累计买满12个月则更新为年会员， 过期每天扣30成长值，只要买过年会员，之后不管怎么买了后期后都显示过期年会员，用yearflag=-1判断，有两个过期时间，一个是年会员过期时间，一个是会员过期时间，累计买满12个月就设置为年会员，并且年会员过期时间一定是以年为单位增加， 检测过期脚本是VSJVIPMonitor每天0点对所有vip用户进行更新(加
成长值),PresentVIP发放会员

涉及表:
privilege_debt_2...   # 会员特权变化
vipinfo							  # 会员信息
vipbuylog							# 每次购买信息 select * from vipbuylog where productid = 'sj_vip_week' order by createtime desc limit 10

# 数数
vipInfo								# 当前会员信息
vipLevelChg						# 可以看到会员成长值变化原因，SELECT vipLevelChg FROM ta.v_event_11 WHERE "$part_date" = '2025-03-24' AND "#event_name" = 'vipLevelChg' and "#account_id" = '211021450' order by "#event_time" desc
~~~

### 排行榜

~~~c++
6.SJ_DIVISION_CH_存储昨天的段位数据，排行榜的星数就是用的这里面的值，也就是star_l，中途星变了，排行榜也不会更新。
5.没有区分先后顺序，按照字典序排序
4.段位榜，如果赛季更新，没有进行赛季继承的玩家不会上榜
3.每天零点从db中获取排名靠前的玩家进行更新
2.排行榜没刷新大概率是因为有缓存了，段位榜是根据段位分计算的(devisioninfo["grade"] * 10000 + devisioninfo["star"])，直接改缓存可能导致星多但段位分没更新。
1.排行榜这些脚本是用的supervisorctl运行的, 'VSJTop':星人榜，'SelectScoreTop':积分榜,'SelectDivisionTop':段位榜单, 线上的脚本服务运行在web1上在。
~~~

### 数数使用

~~~mysql
1. 查询对象可以直接select game_end_info.final_draw_joker_count，不用json解析
...
# 查gamelog信息
SELECT extrainfo, play_seq FROM ta.v_event_11 WHERE "$part_date" = '2025-03-17' AND "#event_name" = 'gamelog' AND "#account_id" = '263529624'  order by "#event_time" desc

# 上报代码逻辑
logoperatorcfg是用户数据
baselogfield1+baselogfield2公共事件属性
real2normfield中放着事件数据的映射关系
返回给大数据的结构中，operation放的是用户数据，data放的是公共事件属性和事件数据。
~~~
...
~~~
7.补签配置sign_config_v1.yaml
6.比如周三第一次登录，第一次签到，也可以补签之前的
5.每周补签不超过5次
4.三个接口：1.签到信息拉取signinfo_v1，2.补签retroactive_v1，3.签到sign_v1
3.漏签的定义是，本周内前几天发生未签到情况，或者上周没有签满
2.连签是累计签到的意思(区分了周、历史累计)，之前签到信息是(SJ_SIGN_DATA_390892不过期和月签到，后来分为了月、周签到，做过数据兼容，第一次签到是没有周的缓存的，就需要生成一下，原理就是用当前日期减去当前是周几，得到上周结束日期，这个日期区分了是否跨月，1.跨月：比如2.1是周三，前两日的签到在一月份，当上线后，比如2.1继续签到因为没有周的缓存，会去同步一月份的数据来更新2.1所在的这一周的缓存，2.没跨月，则直接用当月的数据来生成周缓存。
1.签到分了周和月，每年最后一周如果跨年，会转为上一年的最后一周，因为time.strftime("%W")得到周是从0开始的，比如2025-01-03是周一，是2025年第一周，2025-01-02算出来是第0周，逻辑会转为上年的最后一周，最后一周跨月也是同理


重要参数:
GetCurrentWeekNumber获取当前是第几周
runningdays是历史累计签到的意思，在月签到数据中
...
~~~
1.附属游戏登录：attach_login_v1，比如象棋中登录附属升级
2.获取可展示小游戏信息smallgameinfo_v4
3.进入小游戏：/small/game/check，比如进捕鱼。
4.在象棋中玩升级时，商店里面购买用shop_buy，商品id也是升级的，跟升级APP买东西没有区别。
~~~
...
2.主牌毙主牌时只要最大牌型大过即可
3.大小王>级牌>常主>主花色牌>副牌，主牌虽然有很多种，但比如红桃2和黑桃2并不是对子，必须要同花色，因为每张牌的大小由四部分组成(牌类型(5种):牌越大就越大的值(权重):花色(6种):牌大小)，每部分8位，一共32位，比如红桃2为的大小为：主牌(4):牌越大就越大的值:红桃(2):0，需要第二部分的原因是，比如红桃2，他可能是主2也可能是副2，主2的第二个值就比副2的第二个值更大。不同花色的副2不是拖拉机，因为拖拉机的判断是先挑选seq值一样并且张数为2的，第一步先过滤掉不同花色的，第二步看四部分的前两部分，副2不同花色虽然seq不一样，但前两部分一样。任何一对副2和主2组合都是拖拉机，两对副2就是对子甩牌
...
~~~shell
遇到的案例:
case1:异常情况下在对同一个商品支付时，发生了两次付费(可能是因为ip发生了变化，网络不稳定等原因，应该是苹果的问题)，第一次支付完成后客户端的回调方法中，因为有订单号的缓存，就正常调用了(/appstore/pay/callback)，第二次支付后发现并没有订单号缓存了(第一次完成后就删掉了)，就会走补单的逻辑，因为这种情况和支付后杀端重连的情况是一样的，都是找不到商品id缓存，所以都会走补单逻辑，补单时尽管没有找到对应的补单信息(本次异常情况是因为之前刚好有一笔没付费的订单)，对于后端而言也会创建一个新的订单，总之就是玩家付费了两次就能收到两次发货。

无法改付费金额的渠道：抖音小游戏。



11.主游戏中进入任何游戏付费都是走主游戏下单
10.附属游戏消耗多乐币会走升级自己，搜索GetGoodID
9.在升级里面的附属中充值多乐币(purchase表，用升级的计费点)或者用多乐币买豆(coin_exchange，goodid会拼接游戏名字)，都是记录到升级中。
8.支付常用设备限制，需要平台和业务同时满足，比如非常用设备可能第一次能支付成功，此时平台标记为异常设备，第二次无法支付。
7.buy表中虚拟支付的fee_actual豆为0，如果是多乐币fee不为0，游戏豆fee为0
6.华为沙盒支付只能是最新版本才能用
5.华为：渠道huawei，组huawei， 鸿蒙：渠道huaweihm，组huawei，荣耀：渠道honor，组honor
4.华为、VIVO、荣耀不支持修改商品价格，在他们自己的后台配的，可以用doutest这个商品做测试。实际到账1个多乐币，奖励动画正常。小米内网支付不到账，需要找平台
3.修改计费点金额在rebuilt中，微信小游戏在WechatGetOrderV2中修改
2.订单状态为2表示订单已经支付成功了。订单状态为5，表示已经发奖。
1.苹果下单流程：创建订单(/appstore/getorder)->付款->游戏前端通知业务服务器订单收据(/appstore/pay/callback)，进一步告知支付中心->游戏前端告诉苹果此订单已经完成(此时还未发货)->支付中心通知业务服务器订单完成(/pay/callback)，通知前端可以请求发货了->前端请求发货(/purchase/deliver)。要点说明：升级前端会有一个订单号的内存缓存，正常下单支付后内存缓存的订单号不为空，则会调用(/appstore/pay/callback)通知订单完成，升级前端进一步告诉苹果此订单已完成，之后重新登录时，苹果就不会提醒升级前端需要补单了，如果支付完成后杀端，此时重新登录升级前端会收到苹果推送的已支付但未结算完成的订单，但此时前端发现已经没有订单号的内存缓存了，就会走补单(/appstore/getorder，跟下单参数不一样)，服务器则会找到最近的一笔订单信息(其实也就是玩家刚才支付了的那笔，只是还没更新订单状态)，前端拿到订单号后会调用(/appstore/pay/callback)通知订单完成，后边的流程就一样了。
2.普通andriod下单流程：升级会创建一个订单号，比如SJ_orderid，请求支付中心创建订单，支付中心又会创建一个订单号比如U390892G3T1734155549958，前端会拿着中台给的订单号向第三方请求支付，第三方支付完成后回调支付中心，支付中心会调用/pay/callback并且把SJ_orderid这个订单号发给升级更新订单状态。
3.app和小游戏下单方式不同，app请求第三方下单时会带上支付中心的回调地址，所以比如支付宝能够回调这个地址，但小游戏是在后台配置的地址并且只能写一个，所以用的线上的回调地址，测试环境需要发货就需要去请求。
4.苹果补单(一定是支付完成后杀端)只有苹果有，登录时苹果会主动告诉客户端有哪些单需要补（可以理解为苹果下单成功后会跟支付中心确认此单是否处理完毕，如果没发货，苹果得不到回应，那么苹果会把订单推送给客户端，客户端登录时重新向服务器请求补单），苹果下单和andriod不一样，苹果支付成功后不是直接告诉支付中心，而是告诉客户端一个收据，客户端自己拿这个收据去告诉支付中心。
5.华为补单(类似苹果那套)：是华为SDK实现的补单逻辑，付款完成后仍需在游戏客户端手动点击确认才会告诉支付中心，不点确认就杀端当重新登录时，SDK机制会通知支付中心继续进行发货流程，也就是说没手动点确认前，订单状态由华为维护而不是支付中心维护，旧版本华为补单会向服务器发送/huawei/pay/callback/v2带上单据信息，服务器请求平台，新版本的补单，不请求服务器了，前端直接请求了平台。
6.其他渠道的补单：每次登录都会请求/purchase/deliver，会检测未发货的订单进行发货。
~~~
...
单人匹配:
    ...
    超过3*75找下两个段位
队伍匹配(单人匹配成功后形成队伍):
    三个队伍起配（如果超过100s，则两个队伍起配）
    超过50s后支持 上下段位 找队伍，以50s为单位，50s支持一个段位的浮动，比如100s支持两个段位的浮动。
自由组队找人逻辑：
优先找同段位，找不到则浮动找，先找低于自己段位的，再找高于自己段位的。比如黄金找不到黄金，先找白银再找铂金。 
~~~

礼包功能:

``` shell
8.礼包中缓存记录的奖励只有配置多送的(不记录会员多送)，因为本质上买的是计费点，返回给前端的礼包配置包含了三部分奖励（1.计费点名字如shengjihd_dou3,2.配置多送的3.vip多送的），分别用三个字段表示了，对于后端来说，redis缓存只记录配置多送的，发奖的时候会先发计费点，因为是计费点如果是会员则会把会员多送一起发了，最后发送redis中记录的配置送的奖励。
7.礼包的会员多送比如买50，在52w豆的基础上多送
6.东山再起配置的是两个礼包，目前没有升档逻辑了。
5.目前在用的就只有 结算时的(豆子补给、东山再起、绝甩礼包(会员不推)) 和 入场时的快捷支付
4.快捷支付不会传giftid
3.东山再起的配置是跟房间走的，豆子补给的配置取决于当局输了多少。
2.破产结束触发东山再起，在VSJHandleGiftEvent中，豆子补给有3次(AVOID_LOSE_GIFT_POPTIMES)，豆子补给优先级比东山再起优先级高
...
# 在用的礼包
kGiftIdEnterRoom  = 8,  // 东山再起
kGiftIdLoseFree   = 9,  // 豆子补给
kGiftIdFastBuy    = 12, // 快捷支付
kGiftIdWorryFree  = 13, // url礼包
kGiftIdFirstCharge = 14, // 首充礼包
kGiftIdGoldenEgg = 15, // 砸蛋礼包
```
...
7.前端如果勾选今日不再弹出，用的是客户端本地时间，尽管服务器发了他也不弹。
6.有次数限制的活动，如果当天触发了弹出，会一次性把所有能弹的活动都弹了，当天再加新的活动不会触发弹出了，除非是每次登录都弹出的活动。
5.服务器标签满足时会给前端推送，但活动中会配置clienttag，也就是前端标签，前端会再一次进行过滤。
4.新版活动接口
...
3.recommendinfo是大厅轮播图，activityinfo：所有活动，showtype为1是公告栏，为2是活动，weburl就是活动展示的容器，是外部的url，popuplist可能是弹出的活动
2.活动红点逻辑：1.前端登录时请求web，web请求状态返回结果给前端 2.中台通知web红点状态发生变化，web主动通知前端。
1.部分活动是每个人固定天数的，第一次获取时，发现没拉取，则上报中台，同时web记录玩家该活动缓存天数（比如15天），比如是在活动最后一天领取的，同样会展示15天。也就是说，标签并不需要每次请求都满足，比如回归用户标签，只要触发的时候是回归用户，拉取14天活动，第二天就算不是回归了也会继续展示该活动。

活动红点逻辑：
1.前端登录时请求web，web请求状态返回结果给前端
2.中台通知web红点状态发生变化，web主动通知前端。
...
SELECT transform(ARRAY[1, 2, 3], x -> x * 2) AS doubled_array
19.cross join相当于完全的笛卡尔乘积，不依赖条件，但cross join unset只是展开后与展开的那行联合而不是整个表。
18.unnest展开数组(前边会有隐式的cross join)，https://bi.duole.com/queries/8977/source
...
12.解析json对象，CAST(json_extract_scalar(extrainfo, '$.matchpoint') AS integer) //integer和int没区别，如果是多层字典嵌套的比如{"level1": {"level2": {"matchpoint": "123"}}}可以写成'$.level1.level2.matchpoint'
...
~~~shell
5.同一个设置登录不同账号，session会用同一个，后一个登录的账号覆盖掉前一个的账号。拿到这个session理论上就可以操作这个玩家。
4.中台请求的接口没有cookie，每次都会在session.py中的_load方法中生成一个，然后设置一个无效缓存。
3.session.py中的_processor可以看到每个请求过来后都会重新设置一下session对象，self.store[self.session_id]就是从缓存拿数据，所以每次请求访问web.config._session时都是用的当前请求的session数据。
...
1.SJ_SESSION_cookie 里面有一个session_key, 模拟器请求的时候cookie不一样，比如APP是SJ_SESSION_APP，postman是SJ_SESSION_POSTMAN， 当用postman请求的时候拿到的session_key和SJ_S_390892中存储的不一样(因为APP登录的时候存的是SJ_SESSION_APP的session_key)，所以在postman第一次调用就需要重新登录，每次登录都会刷新SJ_S_390892中存储的值(也就是session_key)
~~~
...
```shell
服务器资源：https://app.yinxiang.com/fx/4df6da82-26d8-4598-ad54-b393821134d6
duolesj:rK0&iA4~hD3!aB3}aE
...
4.牌友邀请都准备后，发给room的MSGID_TEAM_MATCH_TABLE | ID_NTF信息中用的userid是队长的，如果是自由组队发送的userid是-1因为没有队长，会把多乐号小的放队列信息的前边，因为用的有序map。
5.单人匹配和自由组队都是发的teamstart(MSGID_TEAM_START | ID_REQ)，并且只有这个还在用了，MSGID_TEAM_MATCH | ID_REQ没用了。
6.牌友邀请和自由组队在切换时会先发离开(MSGID_LEAVE_GAME | ID_REQ)再发teamstart，从快速匹配切换到组队(部分自由、牌友)时不会先发离开，因为快速匹配这时候还没有队伍的概念，直接发teamstart即可。
...
~~~shell
解放卡住的玩家:
1.如果队伍服没挂,切换新的队伍服,切换到旧的队伍服就能放出卡住的玩家,因为队伍key在服务关闭的时候会删除掉(DissolveTeam)
2.如果挂了,直接启动也能释放出卡住的玩家(ClearTeamData)
...
9.重新连接加入游戏中MSGID_QXGZ_REJOIN_GAME
...
6.匹配成功后才有tableid
5.prematch服务上：DealAutoMatchTimer进行匹配, Join2Table设置SJ_FKLD_T_tableid（记录桌子信息），SJ_FKLD_MATCH_390892（记录玩家tableid，room服务地址，这里面的game服务地址是在room服务算出来的），KEY_FKLD_ROOMSVRD_TM、KEY_FKLD_ROOMSVRD_UM分别记录prematch服务上的tableid和userid，
4.team服务上：SJ_FKLD_TEAM_teamid（记录队伍信息），SJ_FKLD_TEAM_MATCH_390892（记录玩家的teamid，team服务地址），SJ_FKLD_TEAMSVRD_TM、SJ_FKLD_TEAMSVRD_UM是set对象，分别记录team服务上的teamid和userid，
3.prematch是随机选择的
2.在team得到teamid（SJ_TEAMLIST_P0），在prematch得到tableid（SJ_TABLELIST_P0）
1.在team做房间信息检查，如是否开启，是否能进

team服务路径:收到MSGID_QXGZ_MATCH_TABLE-->OnTeamMatchTable-->DealAutoMatch-->SelectPrematchsvrd选择撮合服-->CreateTeam创建队伍key(team和team_match)-->MSGID_QXGZ_MATCH_TABLE_INTERNAL告诉撮合服开始匹配-->MSGID_QXGZ_MATCH_STATUS | ID_NTF通知客户端开始匹配

撮合服路径:收到MSGID_QXGZ_MATCH_TABLE_INTERNAL开始匹配-->TeamMatchTable放进匹配队列mapAutoMatch-->DealAutoMatchTimer定时处理匹配队列-->Join2Table创建桌子key(_MATCH_, _T_)-->MSGID_QXGZ_MATCH_STATUS | ID_NTF通知客户端匹配成功-->MSGID_QXGZ_MATCH_TABLE_INTERNAL | ID_NTF通知队伍服匹配成功-->GetRoomsvrdMsgRoute选择房间服并且更新桌子key的roomsvrd地址-->MSGID_QXGZ_DISTRIBUTE_GAME | ID_NTF通知房间服开始匹配

房间服路径:收到MSGID_QXGZ_DISTRIBUTE_GAME | ID_NTF-->从Player到Table再到Game对象的ProcessAutoJoinGame,最后一个人执行Game对象的ProcessAutoJoinGame时,CheckGameBegin检测到四人全部自己准备完成后调用SetGameStart设置为可以开始游戏状态,最后一个人的Game对象的ProcessAutoJoinGameTable执行完后跳出到Table对象的ProcessAutoJoinGame进入Game对象的OnGameStart-->SelectGamesvrd接着调用UpdateGameRoute更新桌子key的游戏服地址-->MSGID_QXGZ_UPDATE_INTERNAL_ROUTE | ID_NTF通知连接服更新玩家的roomsvrd和gamesvrd地址-->MSGID_QXGZ_DISTRIBUTE_GAME | ID_NTF通知游戏服游戏开始

游戏服路径:收到MSGID_QXGZ_DISTRIBUTE_GAME | ID_NTF-->-->从Player到Table再到Game对象的ProcessAutoJoinGame,四人都进入后从Table对象的ProcessAutoJoinGame进入OnGameStart,跟房间服的方式一样-->MSGID_QXGZ_GAME_INIT | ID_NTF通知客户端游戏开始
~~~
...
1.级牌不生效 级牌不对， 去GameLogic的SetRandomMain添加房间号特殊处理，if(m_GameData.tableInfo.room_id == 11004)，随机主(random_main改为1)才能能配级牌
