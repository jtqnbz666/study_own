...
struct skynet_context {
	...
	void * cb_ud;	//实际的本服务的dispatch_message
	skynet_cb cb;	//每个实例都一样的统一接口 ， 是一个名为 _cb 的函数
...
skynet_callback(struct skynet_context * context, void *ud, skynet_cb cb) {
	context->cb = cb;   //统一的一个 _cb函数，所有服务都是一样的，但具体执行哪个服务的回调函数，取决于下边的 cb_ud.
	context->cb_ud = ud; 	//真正的执行函数的userdata ， 通过这个cb_ud, 实际进入到的只是这个服务的dispatch_message函数，
							//具体执行这个服务的哪个函数，取决于这个服务自己注册的函数类型。可在这个服务的raw_dispatch_message中查看
}
