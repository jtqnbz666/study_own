...
# 连接主机时打印详细信息，需要-v参数
 ssh -v duoletest@152.136.146.64


# 说明：
ssh服务端有严格的权限检测，当家目录对组或其他用户可写时，sshd 认为你的 authorized_keys 路径存在被篡改风险，于是拒绝公钥认证，回退到密码。
~ 必须不可被组/其他写：chmod go-w ~ （g为group，o为other，-w移除写）
~/.ssh 必须 700：chmod 700 ~/.ssh
~/.ssh/authorized_keys 必须 600：chmod 600 ~/.ssh/authorized_keys
上述路径必须归属该用户：chown -R user:user ~/.ssh
```
...
curl http://httpbin.org/ip  # 走8.130.49.219代理访问，如果不export，则需要使用curl --socks5 127.0.0.1:1080 http://httpbin.org/ip才会走代理。
...
3.比如本地通过挂上了vpn能够访问到某个页面，但朋友访问不了，可以远端代理一台中间服务器，让朋友访问这台中间服务器进一步访问到我本地，但有些网页机制比较复杂，比如数数平台，虽然可以登录了，但是还有websocket的检测会失败影响正常使用。如果是http的请求则中间服务器不用特殊处理，但如果是https的请求，中间服务器需要搭建一个Nginx服务器，让朋友用http访问中间服务器，然后中间服务器转为http，配置如下
server {
    listen       8883;
    location / {
        proxy_pass https://127.0.0.1:8000;
    }
}， 这样朋友访问中间服务器ip:8883，然后我本地代理了中间服务器的8000端口，流量就会到我本地的443端口。本地配置是ssh -R 8000:目标网页ip:443 root@8.130.49.219
...
~~~shell
awk语法: awk 'pattern {action}' filename ,parttern {action} 可以有多对.
...
累加某一列的值
awk '{sum += $1} END {print sum}' filename # END表示只在处理最后一行时才输出sum的值。
打印第一行和最后一行
awk 'NR == 1 {print $1} END {print $1}' filename
...
```shell
rsync：增量同步，只传输变化的部分，功能更多（支持排除、压缩、保持属性等）
scp：完整复制，每次都传输整个文件
...
# rsync同步文件
rsync -av -e ssh --exclude='log' ./ duolesj@172.21.65.163:/var/test/bin # --exclude排除
```
...
l +  				# 当前运行行的后序代码
l -  				# 当前运行行的前序代码
l line1, line2 # 看line1到line2行之间的代码
# 实时看代码
先按control + x ，放开后按a
# 移除断点
b ：break 断点
