...
抓住核心数据库表：关系表(只有好友)， 会话表(群组、好友同一张)，消息表(好友和群组是分开的)，群组表(群组详情)，群组成员表(记录群的每个成员)
(在CSessionModel模块)根据不同的用户id 从关系表IMRecentSession中获取对方的ID(可能是用户id，也可能是群组id)， 可以通过表中的type字段区分是哪种类型，表中包含更新时间，当需要更新的时候，判断这个更新时间，如果大于当前时间，则获取这条记录，每次最多获取100条，但通过这个表能得到的消息只有：1.对方id，2.对方类型，3.对方状态(正常或被删除)，还需要获取这个会话的最后一条消息的信息(最后一条消息由谁发的，类型，数据)，如果是好友会话，需要通过自己的id和对方id从关系表中得到relationid(如果是群组就不要做别的操作，因为那个对方id对应的就是groupid)，进一步在好友消息表中获取，(在CSessionModel::fillSessionMsg中，单聊消息调⽤CMessageModel::getLastMsg，群聊消息调⽤CGroupMessageModel::getLastMsg)所以需要查询消息表(分为普通消息表，群消息表，它们都有八张表，前者用relationid%8, 后者用groupid%8)，分别以relationid和groupid为索引，使用orderby字段排序获取最后一条消息。

### 未读消息设计

未读消息存储在redis，分为好友未读和群组未读
1.好友未读, 通过unread_玩家id(hash) 来存储这个玩家与每个好友的未读消息数量
2.群组未读，每个群会记录总消息数量，而群里的每个用户会记录自己在这个群里面的已读消息数量，二者的差值就是未读数量

拓展：虽然未读消息直接放到了redis，但其实也可以存在mysql，如果是好友之间的，直接存到好友关系表，增加两个字段，一个是未读消息用户的id（因为两个用户，只会有一个用户未读，所以只记录一个就行）, 一个是未读消息数量； 如果是群消息，在群组表里记录这个群产生的总消息，群组成员表记录每个成员读取的信息数量，同样是以计算差值的方式。 但实际上redis访问速度更快，并且只放在redis上能避免高并发时mysql和reids消息不一致的问题。

### 实时拉取

1.会话：从会话表里每次只拉取最多一百个会话(以时间排序，记录最后一个会话的时间，下次就读取小于这个时间的， 中途有新会话产生不用考虑拉取问题，因为直接就会更新到客户端)，这个最后一个会话的时间客户端记录就行，如果客户端挂了，那么重头开始拉起就行，并且客户端也可以实现一些缓存机制来避免数据没有记忆而需要重新拉却问题。
2.消息内容：每次拉取100条消息(可以通过消息id来记录当前会话拉取到哪个位置)
...
每个连接给一个BaseSocket对象，里面包含了它的读写缓冲区， 实现一个PDU的类，PDU类里面有一个缓冲区用于去读BaseSocket对象的读缓冲区。
收到一个可读信息时，BaseSocket对象中的读缓冲区会去循环读取套接字中的内容，然后判断读缓冲区里的内容是否大于一个协议头，如果连一个协议头的大小都没有，那肯定因为某些原因数据没有读完，等待下一次读取，如果大于一个协议头则把BaseSocket读缓冲区的第一个字节读出来，因为它代表的是这次PDU的总大小。接着用这个总大小与BaseSocket对象的读缓冲区当前内容的总大小进行对比，如果总大小要大些，还是说明数据没有读取足够，反之则能够判断读取到的数据量是满足这次PDU的条件的， 于是根据总大小把BaseSocket对象中的读缓冲区中的数据拷贝到PDU类中的缓冲区中
在PDU类中自己实现一些功能，为协议头的每个字段以及protobuf压缩后的数据(原数据+协议头偏移量即可)提供一个访问接口