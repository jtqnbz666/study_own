...
//conn_handle是从0开始的， 可以防止sockfd重用引发的冲突
static uint32_t g_conn_handle_generator = 0; //通过它生成conn_handle
CLock CHttpConn::s_list_lock;
list<Response_t*> CHttpConn::s_response_pdu_list; //conn_handle ,content, conten_len
            ...
            if(nFileNameLen <= 255) {
                ...
                if(pPosType != nullptr) {
                    ...
                    if(nTypeLen <= 15) {
                        ...
                        //获取文件内容
                        size_t nPos = m_strContentType.find(BOUNDARY_MARK); //边界的意思
