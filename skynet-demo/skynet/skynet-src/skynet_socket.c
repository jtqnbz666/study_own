...
skynet_socket_poll() {
	...
	switch (type) {				//type是一个整数，当执行forward_message后，具体的执行函数可以在socket.lua中看到
	case SOCKET_EXIT:
		return 0;
	case SOCKET_DATA:
	...
	case SOCKET_CLOSE:
		...
		break;
	case SOCKET_OPEN:			//连接第三方服务  用可写判断是否成功
		forward_message(SKYNET_SOCKET_TYPE_CONNECT, true, &result);
		break;
	case SOCKET_ERR:
		forward_message(SKYNET_SOCKET_TYPE_ERROR, true, &result);
		break;
	case SOCKET_ACCEPT:		//可读
...
skynet_socket_start(struct skynet_context *ctx, int id) {
	uint32_t source = skynet_context_handle(ctx);
	socket_server_start(SOCKET_SERVER, source, id);//id是sockfd一类的
}
