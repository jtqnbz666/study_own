...
static ConnMap_t g_route_conn_map; //fd 与 msgconn对应
typedef hash_map<uint32_t, CUserInfo*> UserInfoMap_t; //user_id 与 用户信息
static UserInfoMap_t g_user_map;    //全局用户表
...
void CRouteConn::Close() {
    ...
    for(UserInfoMap_t::iterator it = g_user_map.begin(); it != g_user_map.end(); ) {
        ...
        CUserInfo* pUser = it_old->second;  //要有一个概念， 一个CUserInfo对应的是一个用户，只是可能包含多种客户端
...
void CRouteConn::HandlePdu(CImPdu* pPdu) {
    switch(pPdu->GetCommandId()) {
        case CID_OTHER_HEARTBEAT:
            //什么也不做，心跳包只是为了更新 m_last_recv_tick
            break;
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu) {
    ...
    if(pUser) {
        ...
        if(user_status == IM::BaseDefine::USER_STATUS_OFFLINE) {
            if(pDBConn) {
                string strSql = "update IMUser_State set state = 2 where user_id = " + int2string(user_id);
            ...
            if(pDBConn) {
                string strSql = "update IMUser_State set state = 1 where user_id = " + int2string(user_id);
                CResultSet* pResult = pDBConn->ExecuteQuery(strSql.c_str());
                pDBManager->RelDBConn(pDBConn);
            }
    ...
    if(CHECK_CLIENT_TYPE_PC(client_type)) {
        ...
        //用户存在
        if(pUser) {
            //如果是pc客户端离线， 但仍然存在pc客户端，则不发送离线通知
            //这种情况是pd客户端多点登录引发的
...
void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu) {
    ...
    uint32_t query_count = msg.user_id_list_size(); //需要查询的个数
    ...
    for(uint32_t i = 0; i < query_count; i ++) {
        ...
        if(pCacheConn) {
            state = "user_id_" + int2string(user_id);
            state = pCacheConn->get(state);
            pCacheManager->RelCacheConn(pCacheConn);
        }
        if(state != "") {
            state_ = string2int(state);
        }
        switch(state_) {
            case 0 : 
            user_stat->set_status(USER_STATUS_OFFLINE);
            break;
            case 1 :
            user_stat->set_status(USER_STATUS_ONLINE);
            break;
            case 2 :
            user_stat->set_status(USER_STATUS_OFFLINE);
            break;
            case 3 :
            user_stat->set_status(USER_STATUS_LEAVE);
            break;
            default: break;
        }
    ...
    if(pUser) {
        if(pUser->FindRouteConn(this)) {
            if(status == USER_STATUS_OFFLINE) {
                pUser->RemoveClientType(client_type);
                 //没有一个客户端,
                if(pUser->IsMsgConnNULL()) { //client_type 个数为0 
                    pUser->RemoveRouteConn(this); //断开这条连接
                    if(pUser->GetRouteConnCount() == 0) {//puser没有在任何一个msgserv上了
                        delete pUser;
                        pUser = NULL;
        ...
        if(status != USER_STATUS_OFFLINE) {
            ...
            if(pUserInfo != NULL) {
                ...
                CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
                if(pDBConn) {
                    string strSql = "insert into IMUser_State values(" + int2string(user_id) + ","  + int2string(status) + ")";
                    CResultSet* pResult = pDBConn->ExecuteQuery(strSql.c_str());
                    pDBManager->RelDBConn(pDBConn);
                }
            } else {
