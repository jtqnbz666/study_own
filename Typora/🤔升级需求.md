...
旧版邮件服务在:/var/www/apps/shengji/Mail

~~~shell
3.如果是模版邮件，去mail_body看template字段，模版配置在MakeMailTemplate.py中
2.一键领取奖励未合并
1.全服邮件还剩多长时间，玩家放到自己的邮箱里面也是多少时间
2.SJ_MAIL_TEMPLATE_CFG 模版类型对应的值
3.redis中outdate表示邮件过期时间，expire表示邮件删除时间，全服邮件删除时间为30天，过期时间取决于duration字段，默认15天
4.redis中单发邮件有效期为15天
5.玩家会记录一个最近拉取全服邮件的时间节点， 如果全服邮件的senddate比最近一次记录的时间大，并且没有过期就会把这个邮件的放到自己的邮箱(将邮件id放到一个list，一个hash记录具体数据)
6.已读的无附件邮件1天后过期
7.查询db的未读邮件
select * from mail_user where userid = 390892 and status = 0 #比如得到247151
8.db根据邮件id获取详细信息
select * from mail_info where ID = 247151
9.db的邮件id是mail_info的自增Key，然后设置给mail_user的mailid字段,如果邮件的内容是一模一样的，mail_info不会增加记录(有一个digest字段判断邮件内容是否一样)，redis的全服邮件的id是自己设置的，普通邮件的id是时间戳和随机数拼起来的
