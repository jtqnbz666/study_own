...
traceback (lua_State *L) {
	const char *msg = lua_tostring(L, 1);		//这里才是真正的传过来的dispatch_message  ，msg 就是 dispatch_message
	if (msg)
		luaL_traceback(L, L, msg, 1);	// 设置了这个当前服务的 dispatch_message函数
...
_cb(struct skynet_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	...
	lua_pushvalue(L,2);		//注意这个细节， 2就是ud，上一步设置了userdata对应的uservalue，所以会调用那个traceback的函数
	...
	const char * self = skynet_command(coFntext, "REG", NULL);
	switch (r) {
	case LUA_ERRRUN:
...
lcallback(lua_State *L) {
	...
	lua_pushcfunction(cb_ctx->L, traceback);		//traceback可以看到绑定了这个当前服务的 dispatch_message函数
	//到此为止， -1是traceback函数， -2是cb_ctx,  -3 是dispatch_message
	lua_setuservalue(L, -2);   //从栈上弹出一个值并将其设为给定索引处用户数据的关联值。(把traceback与userdata(cb_ctx)联系起来), 可以看看traceback的实现   
	lua_setfield(L, LUA_REGISTRYINDEX, "callback_context");	//把userdata(cb_ctx)注册进全局注册表，    每个服务都有独立的注册表
...
luaopen_skynet_core(lua_State *L) {
	...
	lua_getfield(L, LUA_REGISTRYINDEX, "skynet_context");			//每个服务都有一个全局注册表，通过这句话得到ctx实例，并放在栈上
